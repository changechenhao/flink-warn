...
//                .build();
        ValueStateDescriptor<CountWithTimestamp> descriptor = new ValueStateDescriptor<>("myState", CountWithTimestamp.class);
//        descriptor.enableTimeToLive(ttlConfig);
    ...
    public void processElement(
        ...
        if (current == null || current.times.size() == 0) {
            ...
            current.times = new LinkedList<>();
            ctx.timerService().registerProcessingTimeTimer(timer);

        }

        current.times.add(ctx.timestamp());
        // 单词数量加一
        current.count = current.times.size();
    ...
    public void onTimer(
        ...
        while (timestamp - result.times.getFirst() > 10000){
            result.times.removeFirst();
        }
        result.count = result.times.size();
        state.update(result);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 监听本地9999端口，读取字符串
        DataStream<String> socketDataStream = env.socketTextStream("10.2.10.100", 9999);
        ...
        DataStream<Tuple2<String, Long>> timeOutWord = socketDataStream
                ...
                // 将单词作为key分区
                .keyBy(0)
//                .window(SlidingEventTimeWindows.of(org.apache.flink.streaming.api.windowing.time.Time.minutes(2), org.apache.flink.streaming.api.windowing.time.Time.minutes(1)))
